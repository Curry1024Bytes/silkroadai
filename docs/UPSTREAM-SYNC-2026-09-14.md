...
### 随后生产发布（2026-09-14 10:53）

operator 随后明确「部署到线上吧」，已将 `prod@4413e61` 在 VPS 从干净 Git 归档构建并完成发布。上文“待部署”是集成完成时的历史状态；当前实际版本、9.425 秒切换窗口、备份、隔离数据库验收、50 项网络检查、13 用户持久授权和价格不变量证据见 [生产发布报告](DEPLOY-2026-09-14.md)。生产价格页真实数据验收已通过，补齐本地构建数据库缺失时的页面验证；本次仍未启用新 provider 或执行收费请求。
