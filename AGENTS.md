...
## 渠道替换操作改进 2026-09-10（dev，未发布）

- 后台新增「替换渠道」：选择旧/新渠道，预览模型覆盖和影响，再一次迁移模型引用及档次登记。
  保留原有拓扑保护、价格历史和客户 Key；已下架模型也迁移。新-api 仅只读元数据，不修改源码。
- 新接口必须 superadmin；按所选档次租户读取，执行前重核预览指纹，Prisma Serializable 事务
  整体更新；配置过期或并发写冲突要求重新预览。错误提示在编辑弹窗中显示中文及受影响模型。
- 导入预览区分新增模型、映射变化、保留价格；仅有映射变化也可确认。缺失登记渠道保持可见，
  可取消选择；选择改变后需重新预览，所选渠道读取失败时不执行部分写入。
- 完整测试 288 files / 3706 passed / 1 既有 skipped，类型与生产构建通过；本地 Chrome 模拟接口
...
- 上游新增/调整的 19 个测试调用保持原样，另补 20 项故障/接口边界回归；原问题已做旧代码
  ...
  构建通过。首次完整测试因本机 SSH 隧道未恢复而未过；北京时间 2026-09-11 10:26 补验已通过。
- operator 恢复 `127.0.0.1:3000` SSH 隧道后，无凭据状态预检及原 smoke 3/3 通过；完整测试
  290 files / 3736 passed / 1 既有 skipped。只读取 new-api 状态与管理员模型目录，不含收费调用、
  token 轮换或生产配置写入；不代表客户 Key 推理、SDK、支付及供应商真机验收。应用代码未变，
  仅更新验收文档并推送 dev；prod 仍为 `65a99e5`，未部署。
...
- 模型与渠道分组页面统一为「同步 new-api → 预览 → 选择确认」，新增 `/api/admin/newapi-sync`。
  ...
  不写 new-api 计费/渠道/Key。签名绑定租户和双端状态，有效期 10 分钟，写入前再次检查过期。
- 新档次/模型默认停用，启用/上架额外选择；已有价格差异默认不选，价格历史和成本保留。
  明确缺失、归属歧义、缺模型替换只提示，不猜测。固定图片 SKU 和未来生效价不自动覆盖。
- 新取价读取全局 ModelRatio/CompletionRatio/ModelPrice 与真实 GroupRatio，不能再从渠道旧字段
  认定缺价。旧 import API 保留兼容，页面已改走新同步。手动新建 key 可省略并自动生成，
...
## 定价入口修正 2026-09-12（16:19 已上线）

- operator 要求修好后再测试，已发布 `ad8594a`：后台定价每个档次统一“改价并发布”，删除首行重复发布入口，点击明确预选对应档次；历史保留，后端和阶梯计费保护未改。
...
## 成本加价工作台（2026-09-15，dev 开发完成，未部署）

- `/admin/pricing` 新增保存上游报价、充值额度换算、上游倍率、自有加价、批量预览/发布工作台。完整成本独立存储，保存不修改 new-api 或客户售价；旧「批量填成本」更名「旧版参考成本」，保留实现并说明边界。
- 新 migration `20260915090000_add_pricing_cost_rules` 只增加成本草稿/版本历史两表、索引、约束和禁止覆盖历史的触发器。保存使用版本检查和既有发布协调锁；批量最多 30 条，一条失败整批回滚。
- 新 v2 发布意图聚合多模型字典写，沿用持久写日志、API/MySQL 双读和不确定请求保护。全部核验后目录同事务生效；v1 原签名与恢复继续可用。成本配置/版本/映射/规格纳入签名并在写前核对；多字典切换期间仍可能有短暂混合倍率，不宣称 new-api 原子改价。
