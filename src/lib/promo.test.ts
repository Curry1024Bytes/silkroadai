 ...
 *
 * The window is anchored at UTC+8: inclusive `2026-05-10 00:00` through
 * inclusive `2026-06-09 23:59:59.999`. We probe each boundary +/- 1ms (and
 * +/- 1s for sanity) so any timezone slip in `isPromoActive` shows up here.
 */
...
describe('isPromoActive — boundary cases', () => {
    it('is INACTIVE one second before promo start (5/9 23:59:59 UTC+8)', () => {
        const just_before = new Date('2026-05-09T23:59:59+08:00');
    ...
    it('is ACTIVE at the exact promo start instant (5/10 00:00:00.000 UTC+8)', () => {
    ...
    it('treats UTC and UTC+8 as the same instant (no timezone bug)', () => {
        // 2026-05-10 00:00 UTC+8 == 2026-05-09 16:00 UTC. Both must be active.
        expect(isPromoActive(new Date('2026-05-10T00:00:00+08:00'))).toBe(true);
        expect(isPromoActive(new Date('2026-05-09T16:00:00Z'))).toBe(true);
    });
});
...
describe('promo window total length', () => {
    it('spans exactly 31 days (5/10 to 6/10 exclusive)', () => {
