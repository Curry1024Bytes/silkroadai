...
describe('LLmRoute scheduler release gate', () => {
    it('does not schedule or execute work while Batch is disabled', async () => {
        vi.stubEnv('PORTAL_BATCH_ENABLED', 'false');
        vi.useFakeTimers();
        try {
            const { startBatchScheduler } = await import('../worker');
            startBatchScheduler();
            expect(vi.getTimerCount()).toBe(0);
            expect(store.listExpiredBatches).not.toHaveBeenCalled();
            expect(mockFetch).not.toHaveBeenCalled();
        } finally {
            vi.useRealTimers();
            vi.unstubAllEnvs();
