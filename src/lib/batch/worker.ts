 ...
 * → 重启后该行重放(重新计费一次)。LLmRoute 不接受此计费风险,默认关闭入口和调度器,
 * 完成持久执行状态与凭据保护后才能另行启用。
 */
...
export function startBatchScheduler(): void {
    // LLmRoute: opt-in only until durable billing-safe recovery and credential storage are implemented.
    if (process.env.PORTAL_BATCH_ENABLED !== 'true') return;
    if (timer) return;
    const tick = (): void => {
