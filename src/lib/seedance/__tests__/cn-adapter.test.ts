...
describe('seedance-cn adapter submit', () => {
    ...
    it('海外档(promax)http 输入图 → 转存 Cloudflare R2(避免海外上游跨境拉国内图超时)', async () => {
        ...
        expect(b.images[0].url).toMatch(/^https:\/\/images\.llmroute\.club\/seedance-input\//);
    ...
    it('海外档多图【并行】转存:全部转存 R2,顺序 + role 保持不变', async () => {
        ...
        for (const im of b.images) expect(im.url).toMatch(/^https:\/\/images\.llmroute\.club\/seedance-input\//);
    });
