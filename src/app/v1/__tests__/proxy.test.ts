...
describe('/v1 proxy — gpt-image-2-{1,2,4}k fixed-price variants retain billing identity', () => {
    ...
    it('fixed URL verification failure preserves the upstream URL without claiming the target size', async () => {
        ...
        expect(body.data[0]).toMatchObject({ url: upstreamUrl });
    });
    ...
    it('unrecognized fixed output is returned without a fake size or post-charge 502', async () => {
        ...
        expect(body.size).toBeUndefined();
        expect(body.output_format).toBeUndefined();
        expect(body.data[0]).toMatchObject({ b64_json: originalB64 });
    });
...
describe('/v1 proxy — Image 2.5 quality integration', () => {
    ...
    it.each([
        ['gpt-image-2', 'max'],
        ['gpt-image-2-1k', 'xhigh'],
    ])('rejects unsupported %s quality %s before forwarding', async (model, quality) => {
        const res = await POST(
            makeReq('/images/generations', {
                body: { model, prompt: 'a blue square', quality, size: '1024x1024', response_format: 'b64_json' },
            }),
