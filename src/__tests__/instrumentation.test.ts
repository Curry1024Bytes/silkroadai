...
vi.mock('undici', () => ({ setGlobalDispatcher, Agent, ProxyAgent }));
...
describe('instrumentation register()', () => {
    ...
    const origDevProxyUrl = process.env.DEV_PROXY_URL;
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    afterEach(() => {
        ...
        delete process.env.PORTAL_FLAVOR;
        delete process.env.PORTAL_SCHEDULERS;
        if (origDevProxyUrl === undefined) delete process.env.DEV_PROXY_URL;
        else process.env.DEV_PROXY_URL = origDevProxyUrl;
    });
    ...
    it('开发环境配置 DEV_PROXY_URL → 使用代理 dispatcher', async () => {
        process.env.NEXT_RUNTIME = 'nodejs';
        process.env.DEV_PROXY_URL = 'http://127.0.0.1:10808';
        const { register } = await import('../instrumentation');
        await register();
        expect(ProxyAgent).toHaveBeenCalledWith('http://127.0.0.1:10808');
